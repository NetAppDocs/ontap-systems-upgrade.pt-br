...
+
** Para configurações MetroCluster que executam ONTAP 9.7 e versões anteriores, acesse link:other-references.html["Referências"] para acessar _Usando a realocação de agregados para atualizar manualmente o hardware do controlador que executa ONTAP 9.7 ou versões anteriores_.
** Para configurações de IP do MetroCluster e opções adicionais de upgrade para configurações de Fabric MetroCluster, acesse link:other-references.html["Referências"] o conteúdo _MetroCluster Upgrade and Expansion_.




.O que se segue?
Após tomar conhecimento deste procedimento ARL, link:decide_to_use_arl_guide.html["decida se deve usar este procedimento ARL"].
