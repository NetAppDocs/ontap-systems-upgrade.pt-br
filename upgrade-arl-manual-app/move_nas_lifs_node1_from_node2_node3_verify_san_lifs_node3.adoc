...
.Sobre esta tarefa
As LIFs remotas gerenciam o tráfego para os LUNs SAN durante o procedimento de atualização. Mover as LIFs SAN não é necessário para a integridade do cluster ou do serviço durante a atualização. As LIFs SAN não são movidas, a menos que precisem ser mapeadas para novas portas. Você precisa verificar se as LIFs estão íntegras e localizadas nas portas apropriadas após colocar o node3 online.
...
.O que se segue?
Após mover os LIFs de dados NAS pertencentes ao node1 do node2 para o node3 e verificar os LIFs SAN no node3, .
