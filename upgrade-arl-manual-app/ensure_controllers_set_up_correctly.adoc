...
+
Você pode comparar a saída com a saída que você capturou emlink:prepare_nodes_for_upgrade.html#man_prepare_nodes_step29["Passo 29"] de _Prepare os nós para atualização_.
...
+
Acesse link:other-references.html["Referências"] para acessar o _System Administration Reference_ para obter informações sobre os SPs e o _ONTAP 9 Command reference_ para obter informações detalhadas sobre o  `system service- processor network modify` comando.

. Se você deseja configurar um cluster sem switch nos novos nós, vá para link:other-references.html["Referências"] para acessar o _site de suporte da NetApp_ e siga as instruções em _Transição para um cluster sem switch de dois nós_.
...
.O que se segue?
Após confirmar que os novos controladores estão configurados corretamente, link:set_up_storage_encryption_new_controller.html["Configurar a criptografia de armazenamento no novo módulo controlador"].
