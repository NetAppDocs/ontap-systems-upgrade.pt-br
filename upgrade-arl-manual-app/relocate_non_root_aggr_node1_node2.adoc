...
+
A saída exibe  `Done` para um agregado após ele ter sido realocado.
...
Consulte link:other_references.html["Referências"] para acessar o conteúdo _Gerenciamento de disco e agregados com a CLI_ e a _Referência de comandos do ONTAP 9_ para obter mais informações sobre os comandos de realocação de agregados de armazenamento.
...
.O que se segue?
Após realocar agregados não raiz do node1 para o node2, link:move_nas_lifs_node1_node2.html["mover LIFs de dados NAS pertencentes ao node1 para o node2"].
