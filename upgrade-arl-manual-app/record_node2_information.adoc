...
NOTE: Seu sistema pode não ter LIFs intercluster. Você tem uma LIF de gerenciamento de cluster apenas em um nó de um par de nós. Uma LIF de gerenciamento de cluster é exibida na saída de exemplo de link:record_node1_information.html#step1["Passo 1"] em _Record node1 port information_.
...
+
Os domínios de broadcast precisam ser mapeados para as portas do novo controlador posteriormente no procedimento.
...
.O que se segue?
Após registrar as informações do nó 2, link:retire_node2.html["desativar node2"].
