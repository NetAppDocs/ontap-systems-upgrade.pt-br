...
.. À medida que o reparo de acessibilidade é realizado, ONTAP tenta colocar as portas nos domínios de broadcast corretos. No entanto, se ONTAP não conseguir determinar a acessibilidade de uma porta e a porta não corresponder a nenhum dos domínios de broadcast existentes, ONTAP tenta criar novos domínios de broadcast para essas portas. Se necessário, você pode excluir os domínios de broadcast recém-criados se todas as suas portas membro se tornarem portas membro dos grupos de interfaces. Excluir domínios de broadcast:
+
...
. [[man_map_1_step5]]A partir do ONTAP 9.8, o ONTAP modifica automaticamente as portas iniciais das LIFs se as portas forem movidas entre domínios de broadcast durante o procedimento de reparo de acessibilidade da porta de rede. Se a porta inicial de uma LIF foi movida para outro nó ou não está atribuída, essa LIF é apresentada como uma LIF deslocada. Restaure as portas iniciais das LIFs deslocadas cujas portas iniciais não existem mais ou foram realocadas para outro nó.
+
...
.O que se segue?
Após mapear as portas do nó 1 para o nó 3, link:move_nas_lifs_node1_from_node2_node3_verify_san_lifs_node3.html["Mova os LIFs de dados NAS pertencentes ao node1 do node2 para o node3 e verifique os LIFs SAN no node3"].
