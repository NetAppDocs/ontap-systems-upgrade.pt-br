...
Alguns ou todos os agregados foram realocados do node1 para o node2, e a HA ainda está ativada. O node2 assume o volume raiz do node1 e quaisquer agregados não raiz que não foram realocados.

.Sobre esta tarefa
A propriedade dos agregados que foram realocados parece ser a mesma que a propriedade dos agregados não-raiz que foram assumidos, pois o proprietário original não mudou. Quando o nó1 entra no `waiting for giveback state`, o nó2 devolve todos os agregados não-raiz do nó1.

.Passos
. Completolink:relocate_non_root_aggr_node1_node2.html#relocate-step1["Passo 1"] na seção _Relocar agregados não raiz do nó 1 para o nó 2_ novamente.
. Continue com o procedimento de atualização do par de nós.
...
.Sobre esta tarefa
O nó 1 assume o controle de todos os agregados do nó 2, bem como de quaisquer agregados próprios que tenha realocado para o nó 2. Quando o nó 2 entra no `Waiting for Giveback` estado, o nó 1 devolve todos os agregados do nó 2.
...
. Abra node2.
+
Ocorre uma interrupção de cliente para todos os agregados enquanto o node2 está sendo inicializado.
...
O Node3 não assume o controle após uma falha do Node2, pois o HA já está desativado.
...
O nó 2 realocou alguns ou todos os seus agregados para o nó 3, que fornece dados dos agregados que foram realocados. HA está desativado.

.Sobre esta tarefa
Há uma interrupção do cliente para agregados que não foram realocados.

.Passos
. Abra node2.
. Realoque os agregados restantes completando o processo.link:relocate_non_root_aggr_node2_node3.html#verify-partner-sys-id["Passo 1"] atravéslink:relocate_non_root_aggr_node2_node3.html#relocate-step5["Passo 5"] na seção _Relocar agregados não raiz do nó 2 para o nó 3_.
. Continue com o procedimento de atualização do par de nós.
...
Se o node3 falhar enquanto o node2 estiver realocando agregados para o node3, o node2 aborta a realocação de quaisquer agregados restantes.
...
.Passos
. Abra node3.
. Completolink:relocate_non_root_aggr_node2_node3.html#relocate-step5["Passo 5"] novamente na seção _Relocar agregados não raiz do nó 2 para o nó 3_.
. Continue com o procedimento de atualização do par de nós.
...
. Abra node3.
+
Há uma interrupção do cliente para todos os agregados.
...
.Passos
. Abra node4.
. Realoque os agregados restantes que pertenciam ao nó 2, concluindo novamente o processo.link:relocate_node2_non_root_aggr_node3_node4.html#man_relocate_3_4_Step1["Passo 1"] atravéslink:relocate_node2_non_root_aggr_node3_node4.html#step3["Passo 3"] em _Relocar agregados não-raiz do nó 2 do nó 3 para o nó 4_.
. Continue com o procedimento de atualização do par de nós.
