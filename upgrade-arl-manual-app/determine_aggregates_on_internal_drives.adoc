...
NOTE: Antes de prosseguir, você deve colocar os agregados offline e, em seguida, destruí-los nos discos internos. Consulte link:other-references.html["Referências"] para acessar o conteúdo "Gerenciamento de disco e agregados com a CLI" e obter informações sobre como gerenciar agregados.
...
NOTE: Antes de prosseguir, você deve mover os volumes para um gabinete de discos externo, colocar os agregados offline e, em seguida, destruir os agregados nos discos internos. Consulte link:other-references.html["Referências"] para obter o link para o conteúdo "Gerenciamento de discos e agregados com a CLI" e informações sobre como mover volumes.



| Conter volumes raiz nas unidades internas | Não prossiga com este procedimento. Você pode atualizar os controladores consultando link:other-references.html["Referências"] para acessar o _site de suporte da NetApp_ e usando o procedimento _Upgrading the controller hardware on a pair of nodes running clustered Data ONTAP by moving volumes_. 


| Conter volumes não-raiz nas unidades internas e não é possível mover os volumes para armazenamento externo | Não prossiga com este procedimento. Você pode atualizar os controladores usando o procedimento _Atualizando o hardware do controlador em um par de nós executando Clustered Data ONTAP movendo volumes_. Consulte link:other-references.html["Referências"] para acessar o _site de suporte da NetApp_, onde você pode encontrar este procedimento. 
|===


.O que se segue?
Após determinar se o controlador possui agregados em unidades de disco internas, link:prepare_nodes_for_upgrade.html["prepare os nós para atualização"].
