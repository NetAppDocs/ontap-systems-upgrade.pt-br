...
+
Como existe a possibilidade de empate em um cluster com um número par de nós, um nó possui um peso de voto fracionário extra chamado epsilon. Consulte link:other-references.html["Referências"] para obter mais informações no _System Administration Reference_.
...
. Opcionalmente, desative o node1 após a conclusão da atualização. Consulte link:decommission_old_system.html["Desativar o sistema antigo"].


.O que se segue?
Após a desativação do node1, link:install_boot_node3.html["instale e inicialize o node3"].
