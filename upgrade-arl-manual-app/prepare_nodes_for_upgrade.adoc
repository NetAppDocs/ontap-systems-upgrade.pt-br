...
+
Consulte link:other-references.html["Referências"] para obter o link para _Gerenciamento de pares HA_ e siga a seção _Melhores práticas para pares HA_. Nenhum dos nós originais deve estar operando com mais de 50% de utilização; se um nó estiver operando com menos de 50% de utilização, ele pode suportar as cargas de ambos os nós durante a atualização do controlador.
...
Para obter informações sobre como desbloquear as contas de usuário, consulte link:other-references.html["Referências"] para acessar a _Referência de Administração do Sistema_.

====
.. Consulte link:other-references.html["Referências"] para acessar o site de suporte da NetApp e fazer o download do Performance and Statistics Collector (Perfstat Converged).
+
...
. Consulte link:other-references.html["Referências"] para acessar o site de suporte da NetApp e abrir um chamado de suporte no NetApp Support Site.
+
...
+
Você deve verificar fisicamente os node3 e node4 para ver se as baterias NVMEM ou NVRAM estão carregadas. Para obter informações sobre os LEDs do modelo de node3 e node4, consulte link:other-references.html["Referências"] para acessar o _Hardware Universe_.
...
+
Os novos nós devem ter a mesma versão do ONTAP 9.x instalada que está instalada nos nós originais. Se os novos nós tiverem uma versão diferente do ONTAP instalada, você deve inicializar os novos controladores via rede após instalá-los. Para obter instruções sobre como atualizar o ONTAP, consulte link:other-references.html["Referências"] para acessar _Upgrade ONTAP_.
...
....
+
Se algum dos nós estiver em giveback parcial, use o  `storage failover giveback` comando para realizar o giveback e, em seguida, use o  `storage failover show-giveback` comando para garantir que nenhum agregado ainda precise ser devolvido. Para obter informações detalhadas sobre os comandos, consulte link:other-references.html["Referências"] para acessar _HA pair management_.
...
+
Se nem o nó 1 nem o nó 2 possuírem agregados dos quais sejam os proprietários atuais (mas não os proprietários da porta inicial), o sistema retornará uma mensagem semelhante ao exemplo a seguir:
...
....
+
Se algum disco estiver ausente, consulte link:other-references.html["Referências"] para acessar _Disk and aggregate management with the CLI_, _Logical storage management with the CLI_ e _HA pair management_ para configurar o armazenamento para o par de HA.
...
+
Para obter informações detalhadas sobre o  `network interface revert` comando, consulte link:other-references.html["Referências"] para acessar a _ONTAP 9 Command reference_.
...
+
Se algum dos discos apresentar falha, remova-o seguindo as instruções em _Gerenciamento de discos e agregados com a CLI_. (Consulte link:other-references.html["Referências"] para o link para _Gerenciamento de discos e agregados com a CLI_.)
...
+
As VLANs são configuradas em portas físicas. Se as portas físicas forem alteradas, as VLANs precisarão ser recriadas posteriormente no procedimento.
...
+
Se nem iSCSI nem FC estiverem configurados para a SVM, o comando exibirá uma mensagem semelhante ao exemplo a seguir:
...
+
Se o iSCSI ou o FC estiverem configurados para a SVM, o comando exibirá uma mensagem semelhante ao exemplo a seguir:
...
....
+
Os nós em um cluster estão em quorum quando a maioria simples dos nós está íntegra e consegue se comunicar entre si. Para mais informações, consulte link:other-references.html["Referências"] para acessar o _System Administration Reference_.
...
+
Você pode querer reutilizar os parâmetros de rede dos dispositivos de gerenciamento remoto, neste caso os SPs, do sistema original para os SPs nos novos nós. Para obter informações detalhadas sobre o SP, consulte link:other-references.html["Referências"] o _System Administration Reference_ e o _ONTAP 9 Command reference_.
...
....
. Obtenha novas chaves de licença para os novos nós no site de suporte da NetApp. Consulte link:other-references.html["Referências"] para acessar o site de suporte da NetApp.
+
...
Habilite AutoSupport seguindo as instruções no _System Administration Reference_. (Consulte link:other-references.html["Referências"] para o _System Administration Reference_.)
...
+
Para obter informações detalhadas sobre AutoSupport, consulte link:other-references.html["Referências"] para acessar o _System Administration Reference_ e o _ONTAP 9 Command reference_.
...
.O que se segue?
Após preparar os nós para a atualização, link:manage_authentication_okm.html["Gerencie as chaves de autenticação usando o Onboard Key Manager"].
