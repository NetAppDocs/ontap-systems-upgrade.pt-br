...
+
As referências a seguir ajudam você a fazer conexões de cabos adequadas. Acesse link:other-references.html["Referências"] para consultá-las.
...
+
Insira a senha e as informações de backup que você anotou anteriormente no procedimento. Consulte link:manage_authentication_okm.html["Gerencie chaves de autenticação usando o Gerenciador de chaves integrado"].
...
+
O  `disk reassign`comando reatribui apenas os discos para os quais  `_node2_sysid_`é o proprietário atual.
...
.O que se segue?
Após instalar e inicializar o node4, link:set_fc_uta_uta2_config_node4.html["defina a configuração FC ou UTA/UTA2 no node4"].
