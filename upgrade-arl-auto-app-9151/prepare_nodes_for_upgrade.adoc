...
summary: 'Prepare os nós e execute uma série de verificações prévias ao atualizar para controladores introduzidos no ONTAP 9.15.1 ou posterior usando  `system controller replace` comandos.' 
---
...
[role="lead"]
O processo de substituição do controlador começa com uma série de verificações prévias. Você também coleta informações sobre os nós originais para uso posterior no procedimento. Se alguma verificação prévia falhar, você deve corrigir a propriedade do agregado antes de prosseguir com a atualização.



[[step-1-prepare-node1-and-node2]]
== Etapa 1: prepare o nó 1 e o nó 2

Inicie o processo de substituição do controlador e execute verificações prévias no node1 e no node2. Se necessário, determine o tipo de unidades de autocriptografia em uso.
...
[NOTE]
====
Você só pode executar o `system controller replace start` comando no nível de privilégio avançado:
...
====
+
Você verá uma saída semelhante ao exemplo a seguir. A saída exibe a versão do ONTAP em execução no seu cluster:
...
....
. Selecione  `y`, você verá a seguinte saída:
+
....
...
| Verificação da versão da imagem | Verifica se os nós que estão sendo substituídos executam a mesma versão do ONTAP. Se as versões da imagem do ONTAP forem diferentes, a tarefa falha. Os novos nós devem ter a mesma versão do ONTAP 9.x instalada neles que está instalada nos nós originais. Se os novos nós tiverem uma versão diferente do ONTAP instalada, você precisa inicializar os novos controladores via rede após instalá-los. Para instruções sobre como atualizar o ONTAP, consulte link:other-references.html["Referências"] para link para _Upgrade ONTAP_. 
...
| Verificação do estado do disco | Se algum dos nós que estão sendo substituídos tiver discos ausentes ou com falha, a tarefa falhará. Se algum disco estiver ausente, consulte link:other-references.html["Referências"] para acessar _Disk and aggregate management with the CLI_, _Logical storage management with the CLI_ e _HA pair management_ para configurar o armazenamento para o par de HA. 
...
| Verificação de status do AutoSupport | Se as notificações do AutoSupport não estiverem configuradas, a tarefa falhará. Você deve ativar o AutoSupport antes de iniciar o procedimento de substituição do controlador. 
...
== Etapa 2: Corrija a propriedade agregada se uma verificação prévia do ARL falhar
...
----
. Reinicie o processo de substituição do controlador:
+
[source, cli]
----
system controller replace start -nodes <node_names>
...
== Etapa 3: Prepare o node3 e o node4

Verifique se a build nos nós 3 e 4 é uma imagem NODAR (sem criptografia de dados em repouso). Se a build for uma imagem NODAR, você precisa instalar uma imagem DARE (criptografia de dados em repouso).

.Sobre esta tarefa
Execute as seguintes etapas nos nós 3 e 4.
...
systemshell
----
. [[check-build]]Verifique se a compilação é uma imagem NODAR:
+
[source, cli]
----
cat /BUILD
----
+
Na saída do comando, verifique se  `build-type` é uma imagem NODAR, por exemplo,  `DEBUG SIM NODAR`.

+

NOTE: Se  `build-type` não for uma imagem NODAR, a compilação será uma imagem DARE.

. Se a compilação for uma imagem NODAR, instale uma imagem DARE.
+
.. Saia do shell do sistema e do menu de inicialização:
+
[source, cli]
----
halt
----
.. Instale uma imagem DARE. Para obter mais informações, consulte o artigo da Knowledge Base link:https://kb.netapp.com/on-prem/ontap/Ontap_OS/OS-KBs/Netbooting_a_FAS_and_AFF_Controller_in_ONTAP_9["Inicializando pela rede um controlador FAS e AFF no ONTAP 9"^].




.O que se segue?
Após preparar os nós, link:complete-preparation-for-upgrade.html["Conclua a preparação para a atualização"].
