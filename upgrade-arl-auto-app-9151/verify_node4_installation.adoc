...
summary: 'Verifique se as portas físicas do nó 2 estão mapeadas corretamente para o nó 4 e restaure a configuração de rede no nó 4 ao atualizar para controladores introduzidos no ONTAP 9.15.1 ou posterior usando  `system controller replace` comandos.' 
---
...
[role="lead"]
Você deve verificar se as portas físicas do nó2 estão mapeadas corretamente para as portas físicas do nó4. Isso permite que o nó4 se comunique com outros nós no cluster e com a rede após a atualização.
...
Se as portas físicas no nó 2 não corresponderem diretamente às portas físicas no nó 4, a seção seguinte <<restore_network_configuration_on_node4_9151,Restaure a configuração de rede no node4>> deve ser utilizada para reparar a conectividade de rede.
...
Neste ponto do procedimento, a operação é pausada enquanto o node4 entra no quórum.



[[step-1-verify-that-node4-is-in-quorum]]
== Passo 1: Verifique se o nó 4 está em quórum

Verifique se o nó 4 está em quorum. Em seguida, retome a operação de substituição do controlador.
...
== Etapa 2: restaure a configuração de rede no nó4
...
.O que se segue?
Após verificar a instalação do node4, link:restore_key-manager_config_node4.html["Restaurar a configuração do gerenciador de chaves no node4"].
