...
summary: 'Instale e inicialize o node4 e instale o software ONTAP ao atualizar para controladores introduzidos no ONTAP 9.15.1 ou posterior usando os comandos  `system controller replace`.' 
---
...
+
Conecte os cabos das seguintes conexões, usando as instruções nas _Instruções de instalação e configuração_ para a plataforma node4, o documento apropriado da prateleira de disco e o _Gerenciamento de pares de HA_.
...
. [[auto_check_4_step11]]Quando o node4 parar no menu de inicialização, reatribua os discos do node2 ao node4 executando o seguinte comando no node4:
+
...
NOTE: No exemplo de saída do console acima, ONTAP solicita o nome do nó parceiro se o sistema usar discos com Particionamento Avançado de Disco (ADP).

. [[auto_check_4_step27]]No prompt do LOADER, inicialize o nó:
+
`boot_ontap menu`
...
.O que se segue?
Após instalar e inicializar o node4, link:verify_node4_installation.html["Verifique a instalação do node4"].
