...
summary: 'Informe à NetApp que o sistema antigo não está mais em operação após a atualização para os controladores introduzidos no ONTAP 9.15.1 ou posterior, utilizando os comandos  `system controller replace`.' 
---
...
.Passos
. Consulte link:other-references.html["Referências"] para acessar o _NetApp Support Site_ e faça login.
...
.O que se segue?
Após a desativação do sistema antigo, link:resume_snapmirror_operations.html["retomar SnapMirror operações"].
