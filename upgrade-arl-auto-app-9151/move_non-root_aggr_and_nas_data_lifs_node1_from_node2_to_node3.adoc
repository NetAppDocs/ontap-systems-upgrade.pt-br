...
summary: 'Verifique se as LIFs de dados NAS do node1 no node2 foram realocadas do node2 para o node3 ao atualizar para controladores introduzidos no ONTAP 9.15.1 ou posterior usando  `system controller replace` comandos.' 
---
...
.Sobre esta tarefa
Durante o procedimento de atualização, as LIFs remotas gerenciam o tráfego para os LUNs SAN. A movimentação das LIFs SAN não é necessária para a integridade do cluster ou do serviço durante a atualização. As LIFs SAN não são movidas, a menos que precisem ser mapeadas para novas portas. Você verifica se as LIFs estão íntegras e localizadas nas portas apropriadas após colocar o node3 online.
...
. Caso necessário, restaure e reverta as LIFs deslocadas ou migre e modifique manualmente as LIFs do nó 1 que não foram realocadas automaticamente para o nó 3.
+
[role="tabbed-block"]
====
...
--
.. Migre os LIFs que não foram realocados automaticamente para o nó 3:
+
[source, cli]
----
...
.O que se segue?
Após mover agregados não raiz e LIFs de dados NAS pertencentes ao node1 do node2 para o node3, link:relocate_non_root_aggr_nas_lifs_from_node2_to_node3.html["Realocar agregados não raiz e LIFs de dados NAS do nó2 para o nó3"].
