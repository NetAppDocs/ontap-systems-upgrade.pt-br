...
[role="lead"]
Existem diversos métodos de realocação agregada (ARL) para atualizar o hardware do controlador. Este procedimento de ARL descreve como atualizar um par de HA de controladores em sistemas de armazenamento AFF e FAS introduzidos no ONTAP 9.15.1 ou posterior com novos controladores, mantendo os discos existentes e os dados armazenados nos discos.


NOTE: Não é possível usar este procedimento para atualizar uma configuração MetroCluster FC ou IP. Para atualizar uma configuração MetroCluster, consulte link:other-references.html["Referências"] a documentação _MetroCluster Upgrade and Expansion_.
...
Durante uma atualização de controlador, o controlador existente é substituído por outro que executa uma plataforma mais recente ou mais poderosa. Este conteúdo descreve os passos para o procedimento parcialmente automatizado, que utiliza verificações automáticas de acessibilidade da porta de rede para simplificar ainda mais a experiência de atualização do controlador.

.O que se segue?
Após tomar conhecimento deste procedimento de atualização do ARL, link:decide_to_use_the_aggregate_relocation_guide.html["decidir se deve usar este procedimento"].
