...
summary: 'Caso necessário, configure a criptografia de armazenamento, instale certificados SSL e configure servidores de gerenciamento de chaves ao atualizar para controladores introduzidos no ONTAP 9.15.1 ou posterior usando os comandos  `system controller replace`.' 
---
...
.O que se segue?
Após configurar a criptografia de armazenamento no novo módulo controlador, link:set_up_netapp_volume_encryption_new_module.html["Configurar a criptografia de Volume ou Aggregate da NetApp no novo módulo de controlador"].
