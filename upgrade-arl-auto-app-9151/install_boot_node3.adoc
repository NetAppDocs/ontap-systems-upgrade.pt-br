...
summary: 'Instale e inicialize o node3 e instale o software ONTAP ao atualizar para controladores introduzidos no ONTAP 9.15.1 ou posterior usando  `system controller replace` comandos.' 
---
...
. Digite  `Ctrl-C` durante o autoboot para interromper o nó no prompt  `LOADER>`.
...
.O que se segue?
Após instalar e inicializar o node3, link:verify_node3_installation.html["Verifique a instalação do node3"].
