...
[role="lead"]
Existem diversos métodos de realocação agregada (ARL) para atualizar o hardware do controlador. Este procedimento de ARL descreve como atualizar um par de HA de controladoras em sistemas de armazenamento AFF e FAS introduzidos no ONTAP 9.15.1 ou posterior com novos controladores, mantendo os discos e dados armazenados existentes. Este é um procedimento complexo que deve ser utilizado apenas por administradores experientes do ONTAP.
