...
summary: 'Decida se deseja usar o ARL para atualizar para controladores introduzidos no ONTAP 9.15.1 ou posterior usando  `system controller replace` comandos.' 
---
...
[role="lead"]
Existem diversos métodos de realocação agregada (ARL) para atualizar o hardware do controlador. Este procedimento de ARL descreve como atualizar um par de HA de controladores em sistemas de armazenamento AFF e FAS para controladores introduzidos no ONTAP 9.15.1 ou posterior, mantendo os discos existentes e os dados armazenados nos discos. Este é um procedimento complexo que deve ser utilizado apenas por administradores experientes de ONTAP.
...
|===
+
Para realizar uma atualização listada na tabela acima, consulte link:other-references.html["Referências"] para acessar _Use "system controller replace" commands to upgrade controller models in the same chassis_.

* Você está atualizando uma configuração MetroCluster FC ou IP. Para atualizar uma configuração MetroCluster, consulte link:other-references.html["Referências"] a documentação de atualização e expansão do MetroCluster.
...
.O que se segue?
Após decidir utilizar este procedimento ARL, link:required_tools_and_documentation.html["Saiba mais sobre as ferramentas e a documentação necessárias"].
