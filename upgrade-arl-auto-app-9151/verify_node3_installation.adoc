...
summary: 'Verifique se as portas físicas do node1 estão mapeadas corretamente para o node3 e restaure a configuração de rede no node3 ao atualizar para controladores introduzidos no ONTAP 9.15.1 ou posterior usando  `system controller replace` comandos.' 
---
...
[role="lead"]
Você deve verificar se as portas físicas do node1 estão mapeadas corretamente para as portas físicas do node3. Isso permite que o node3 se comunique com outros nós no cluster e com a rede após a atualização.

.Sobre esta tarefa
Consulte link:other-references.html["Referências"] para acessar o _Hardware Universe_ e obter informações sobre as portas nos novos nós. Você usará essas informações posteriormente nesta seção.

O layout físico das portas pode variar, dependendo do modelo dos nós. Quando o novo nó é inicializado, ONTAP tenta determinar quais portas devem hospedar as LIFs do cluster para que ele entre automaticamente em quorum.

Se as portas físicas no nó1 não corresponderem diretamente às portas físicas no nó3, a seção subsequente <<restore_network_configuration_on_node3_9151,Restaure a configuração de rede no node3>> deve ser utilizada para reparar a conectividade da rede.
...
Neste ponto do procedimento, a operação é pausada enquanto o node3 entra no quórum.



[[step-1-verify-that-node3-is-in-quorum]]
== Passo 1: Verifique se o nó3 está em quórum

Verifique se o nó 3 está em quorum. Em seguida, retome a operação de substituição do controlador.
...
. A substituição do controlador é interrompida para intervenção com a seguinte mensagem:
+
....
...
== Etapa 2: restaure a configuração de rede no nó3
...
Para obter mais informações sobre como criar e recriar VLANs, grupos de interfaces e domínios de broadcast, consulte link:other-references.html["Referências"] para acessar _Network Management_.
...
+
À medida que o reparo de acessibilidade é realizado, ONTAP tenta alocar as portas nos domínios de broadcast corretos. No entanto, se ONTAP não conseguir determinar a acessibilidade de uma porta e a porta não pertencer a nenhum dos domínios de broadcast existentes, ONTAP cria novos domínios de broadcast para essas portas.
...
.O que se segue?
Após verificar a instalação do node3, link:restore_key-manager_configuration_node3.html["Restaurar a configuração do gerenciador de chaves no node3"].
