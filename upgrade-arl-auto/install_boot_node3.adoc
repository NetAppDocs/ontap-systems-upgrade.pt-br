...
+
Se você não tiver uma configuração FC ou UTA/UTA2, execute link:reassign-node1-disks-to-node3.html#reassign-node1-node3-step1["Reatribuir discos node1 a node3, passo 1"] para que o node3 possa reconhecer os discos do node1.

. [[auto9597_istall3_step29]]para uma configuração MetroCluster, sistemas e sistemas da série V com software de virtualização FlexArray conetado a matrizes de armazenamento, vá para link:reassign-node1-disks-to-node3.html#reassign-node1-node3-step1["Reatribuir discos node1 a node3, passo 1"].
