...
[role="lead"]
Você deve verificar se as portas físicas no nó1 estão mapeadas corretamente para as portas físicas no nó3. Isso permite que o nó3 se comunique com outros nós no cluster e com a rede após a atualização.
...
As configurações de porta podem variar dependendo do modelo dos nós. Você deve tornar a configuração de porta e LIF no nó original compatível com o uso e a configuração planejados para o novo nó. Isso ocorre porque o novo nó reproduz a mesma configuração ao inicializar, o que significa que, ao inicializar o node3, ONTAP tenta hospedar LIFs nas mesmas portas que eram usadas no node1.

Portanto, se as portas físicas no node1 não corresponderem diretamente às portas físicas no node3, serão necessárias alterações na configuração do software para restaurar a conectividade do cluster, do gerenciamento e da rede após o boot. Além disso, se as portas do cluster no node1 não corresponderem diretamente às portas do cluster no node3, o node3 poderá não se reintegrar automaticamente ao quorum ao ser reinicializado, até que você altere a configuração do software para hospedar as LIFs do cluster nas portas físicas corretas.
...
+
Consulte link:other-references.html["Referências"] para acessar _Gerenciamento de Rede_ ou o _ONTAP 9 Command reference_ para mais informações.
