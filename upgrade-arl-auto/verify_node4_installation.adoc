...
CAUTION: Nesta fase, a configuração MetroCluster não está em estado normal e você pode ter erros a resolver. Consulte link:verify_health_of_metrocluster_config.html["Verifique a integridade da configuração do MetroCluster"].
...
.Sobre esta tarefa
Para obter mais informações sobre como criar e recriar VLANs, grupos de interfaces e domínios de broadcast, acesse link:other-references.html["Referências"] e clique no link para _Gerenciamento de Rede_.
...
Se você estiver usando NetApp Aggregate Encryption (NAE) ou NetApp Volume Encryption (NVE) para criptografar volumes no sistema que está atualizando, a configuração de criptografia deve ser sincronizada com os novos nós. Se você não restaurar key-manager, ao realocar os agregados do node2 do node3 para o node4 usando ARL, os volumes criptografados serão colocados offline.
