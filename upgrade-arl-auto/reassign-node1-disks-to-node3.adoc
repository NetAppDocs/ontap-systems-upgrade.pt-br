...
.Passos
. [[reassign-node1-node3-step1]]Verifique se o node1 parou no menu de inicialização e reatribua os discos do node1 ao node3:
+
...
----
====
. Se o sistema entrar em um loop de reinicialização com a mensagem `no disks found` , isso ocorre porque ele redefiniu as portas de volta para o modo de destino e, portanto, não consegue ver nenhum disco.  Executar<<auto_check3_step3,Passo 3>> para<<auto_check3_step8,Passo 8>> no node3 para resolver esse problema.
...
. [[auto_check3_step9]]Se você estiver atualizando de um sistema com discos externos para um sistema que suporta discos internos e externos (sistemas AFF A800 , por exemplo), defina o agregado node1 como o agregado raiz para confirmar que o node3 inicializa a partir do agregado raiz do node1.  Para definir o agregado raiz, vá ao menu de inicialização no nó 3 e selecione a opção `5` para entrar no modo de manutenção.
+
