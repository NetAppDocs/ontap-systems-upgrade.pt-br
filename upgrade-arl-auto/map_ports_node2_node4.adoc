...
[role="lead"]
Você deve verificar se as portas físicas no nó2 estão mapeadas corretamente para as portas físicas no nó4. Isso permite que o nó4 se comunique com outros nós no cluster e com a rede após a atualização.
...
+
Consulte link:other-references.html["Referências"] para acessar _Network Management_ ou o _ONTAP 9 Command reference_ e veja _Configuring failover settings on a LIF_ para mais informações.
