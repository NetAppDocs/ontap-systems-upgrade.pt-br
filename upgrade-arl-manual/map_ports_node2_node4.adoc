...
[role="lead"]
As portas físicas no nó 2 devem ser mapeadas corretamente para as portas físicas no nó 4. Isso permite que o nó 4 se comunique com outros nós no cluster e com a rede após a atualização.

.Antes de começar
Você já deve ter informações sobre as portas nos novos nós, para acessar essas informações, consulte link:other-references.html["Referências"] para o link para o _Hardware Universe_. Você usará essas informações mais adiante nesta seção.
...
.Sobre esta tarefa
As configurações de porta podem variar dependendo do modelo dos nós. Você deve garantir que a configuração de porta e LIF do nó original seja compatível com a configuração planejada para o novo nó. Isso ocorre porque o novo nó reproduz a mesma configuração ao inicializar, ou seja, ao inicializar o nó4, Data ONTAP tenta hospedar as LIFs nas mesmas portas que eram usadas no nó2.

Portanto, se as portas físicas no node2 não corresponderem diretamente às portas físicas no node4, serão necessárias alterações na configuração do software para restaurar a conectividade do cluster, do gerenciamento e da rede após a inicialização. Além disso, se as portas do cluster no node2 não corresponderem diretamente às portas do cluster no node4, o node4 poderá não se reintegrar automaticamente ao quorum ao ser reinicializado, até que seja feita uma alteração na configuração do software para hospedar as LIFs do cluster nas portas físicas corretas.
...
+
Para obter mais informações, consulte link:other-references.html["Referências"] para link para _Gerenciamento de Rede_ ou o _ONTAP 9 Command reference_, e acesse _Configuring failover settings on a LIF_.
