...
summary: Solucionar falhas de realocação de agregados que podem ocorrer durante uma atualização manual do ARL em controladores que executam o ONTAP 9.7 ou anterior. 
---
...
+
Para obter informações detalhadas sobre as  `storage aggregate relocation start`, `override-vetoes` e  `override-destination-checks` opções, consulte link:other_references.html["Referências"] para o link para a _ONTAP 9 Command reference_.
...
* Durante a Etapa 3, os agregados são realocados do nó2 para o nó3. Alguns dos agregados que estão sendo realocados têm o nó1 como seu nó inicial. Por exemplo, um agregado desse tipo poderia ser chamado aggr_node_1. Se a realocação de aggr_node_1 falhar durante a Etapa 3 e a realocação não puder ser forçada, o agregado permanecerá no nó2.
* Após o Estágio 4, quando o nó2 é substituído pelo nó4. Quando o nó2 é substituído, aggr_node_1 entra em operação com o nó4 como sua porta inicial em vez do nó3.
