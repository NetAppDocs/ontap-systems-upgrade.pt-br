...
* Este procedimento aplica-se a configurações MetroCluster de quatro e oito nós executando ONTAP 9.5 e versões anteriores. Para configurações MetroCluster executando ONTAP 9.6 e versões posteriores, acesse link:other-references.html["Referências"] para o link _Usando os comandos "system controller replace" para atualizar o hardware do controlador executando ONTAP 9.5 para ONTAP 9.7_.
