...
* Se você estiver atualizando um sistema com discos de armazenamento, precisará concluir toda esta seção e, em seguida, acessar as link:set_fc_or_uta_uta2_config_on_node3.html#configure-fc-ports-on-node3["Configurar portas FC no node3"] e link:set_fc_or_uta_uta2_config_on_node3.html#check-config-uta-uta2-node3["Verifique e configure as portas UTA/UTA2 no node3"] seções e inserir comandos no prompt do cluster.
...
NOTE: Se você estiver atualizando para um sistema com ambos os nós no mesmo chassi, instale o nó 4 e o nó 3 no chassi. Se você não instalar os dois nós no mesmo chassi, ao inicializar o nó 3, ele se comportará como se estivesse em uma configuração de chassi duplo e, ao inicializar o nó 4, a interconexão entre os nós não será estabelecida.

. [[auto_install3_step3]]Cabo node3, movendo as conexões de node1 para node3.
+
Conecte as seguintes conexões usando as _Instruções de instalação e configuração_ para a plataforma node3, o documento de prateleira de disco apropriado e a documentação de _Gerenciamento de pares de HA_.

+
Consulte link:other-references.html["Referências"] para o link para _Gerenciamento de pares HA_.
...
+
Insira a senha e as informações de backup que você anotou anteriormente no procedimento. Consulte link:complete-preparation-for-upgrade.html#manage_storage_encryption_using_okm["Gerencie a criptografia de armazenamento usando o Gerenciador de chaves integrado"].
...
.O que se segue?
* Se você tiver um sistema com uma configuração FC ou UTA/UTA2,link:set_fc_or_uta_uta2_config_on_node3.html["definir e configurar as portas FC ou UTA/UTA2 no nó3"] .
* Se você não tiver uma configuração FC ou UTA/UTA2,link:reassign-node1-disks-to-node3.html#reassign-node1-node3-app-step1["reatribuir discos do nó1 ao nó3, Etapa 1"] para que o nó3 possa reconhecer os discos do nó1.
* Se você tiver uma configuração MetroCluster ,link:set_fc_or_uta_uta2_config_on_node3.html["definir e configurar as portas FC ou UTA/UTA2 no nó3"] para detectar os discos anexados ao nó.
