...
Se as portas físicas no nó1 não corresponderem diretamente às portas físicas no nó3, a seção subsequente <<restore_network_configuration_on_node3,Restaure a configuração de rede no node3>> deve ser utilizada para reparar a conectividade da rede.
...
.O que se segue?
Após verificar a instalação do node3, link:restore_key-manager_configuration_node3.html["restaure a configuração do gerenciador de chaves no nó 3"].
