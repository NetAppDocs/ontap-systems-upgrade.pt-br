...
.Passos
. [[reassign-node2-node4-app-step1]]Verifique se o node2 parou no menu de inicialização e reatribua os discos do node2 ao node4:
+
...
+
** Executar<<reassign-node2-node4-app-step3,Passo 3>> para<<reassign-node2-node4-app-step8,Passo 8>> no nó 4
** Ir para a seçãolink:verify_node4_installation.html["Verifique a instalação do node4"]
...
. [[reassign-node2-node4-app-step9]] Se você estiver atualizando de um sistema com discos externos para um sistema que suporta discos internos e externos (sistemas AFF A800 , por exemplo), defina o agregado node2 como o agregado raiz para garantir que o node4 inicialize a partir do agregado raiz do node2.  Para definir o agregado raiz, vá ao menu de inicialização no node4 e selecione a opção `5` para entrar no modo de manutenção.
+
...
.O que se segue?
Após reatribuir os discos do nó 2 ao nó 4, link:verify_node4_installation.html["Verifique a instalação do node4"].
