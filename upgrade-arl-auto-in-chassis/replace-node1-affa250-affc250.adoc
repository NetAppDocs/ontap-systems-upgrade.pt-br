...
---
= Substitua o módulo controlador node1 AFF A250 ou AFF C250
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Para remover o módulo controlador node1, remova o dispositivo de gerenciamento de cabos e destrave as travas de segurança. Em seguida, remova o módulo controlador do chassi.
...
+
Se você não interromper o AUTOBOOT a tempo e o nó 1 começar a inicializar, aguarde o prompt e pressione Ctrl+C para entrar no menu de inicialização. Depois que o nó parar no menu de inicialização, use a opção  `8` para reinicializar o nó e interromper o AUTOBOOT durante a reinicialização.
...
.O que se segue?
Após instalar o módulo controlador AFF A30, AFF A50, AFF C30 ou AFF C60, link:netboot-node1.html["netboot node1"].
