...
.Sobre esta tarefa
Após inicializar o node2 com o módulo de sistema de substituição, você verifica se ele foi instalado corretamente. Você deve aguardar até que o node2 entre no quorum e então retomar a operação de substituição do controlador.
...
== Passo 1: verifique se o nó2 está em quórum

Verifique se o nó2 está em quorum. Em seguida, retome a operação de substituição do controlador.
...
. Com a substituição do controlador em estado pausado, prossiga para <<restore_network_configuration_on_node2,Restaure a configuração de rede no node2>>.
...
== Etapa 2: restaure a configuração de rede no nó 2
...
.Sobre esta tarefa
Para obter mais informações sobre como criar e recriar VLANs, grupos de interfaces e domínios de broadcast, consulte link:other-references.html["Referências"] para acessar o conteúdo _Network Management_.
...
.O que se segue?
Após verificar a instalação do node2, link:restore-key-manager-config-node2.html["restaurar a configuração do gerenciador de chaves no node2"].
