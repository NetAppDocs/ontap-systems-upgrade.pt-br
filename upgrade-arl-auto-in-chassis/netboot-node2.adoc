...
summary: Ao usar o ARL para atualizar modelos de controladores no mesmo chassi, você deve netboot o node2 após trocar o módulo de sistema de substituição correspondente. 
---
= Netboot node2
:hardbreaks:
:allow-uri-read: 
...
.O que se segue?
Após inicializar o nó2 pela rede, link:cable-node2-for-shared-cluster-HA-storage.html["cabo do nó2 para cluster-HA compartilhado e armazenamento"].
