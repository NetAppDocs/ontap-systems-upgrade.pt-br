...
* Este procedimento é complexo e pressupõe que você possua habilidades avançadas de administração do ONTAP. Você deve ler e compreender o link:guidelines_for_upgrading_controllers_with_arl.html["diretrizes para atualização de controladores"] e o link:overview_of_the_arl_upgrade.html["Sequência de atualização do ARL"] antes de iniciar a atualização.
...
* Se você estiver atualizando de um sistema existente com tempo de inatividade incluído no link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["matriz de sistemas suportados"], você pode atualizar o hardware do controlador movendo o armazenamento ou entrando em contato com o suporte técnico. Consulte link:other_references.html["Referências"] para obter o link para _Atualizar movendo volumes ou armazenamento_.
