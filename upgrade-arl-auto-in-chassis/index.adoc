...
[role="lead"]
Existem diversos métodos de realocação agregada (ARL) para atualizar o hardware do controlador. Este procedimento descreve como atualizar controladores de armazenamento em um par de HA convertendo o sistema existente para o sistema de substituição, mantendo o chassi existente, os discos e os dados armazenados nos discos.
...
NOTE: Os termos *node1* e *node2* são usados apenas como referência aos nomes dos nós neste documento. Ao seguir o procedimento, você deve usar os nomes reais dos seus nós.



== Requisitos e limitações

Você precisa considerar fatores importantes antes de iniciar a atualização.


NOTE: Você deve revisar todas as seguintes informações importantes.

* Este procedimento é complexo e pressupõe que você possua habilidades avançadas de administração do ONTAP. Você deve ler e compreender o link:guidelines-for-upgrading-controllers-with-arl.html["diretrizes para atualização de controladores"] e o link:overview-of-the-arl-upgrade.html["Sequência de atualização do ARL"] antes de iniciar a atualização.
...
* Se você tiver um switch que não seja compatível com a versão do ONTAP e com o sistema de substituição para o qual você está atualizando, consulte link:other-references.html["Referências"] para acessar o _Hardware Universe_.
...
* Se você estiver atualizando de um sistema existente com tempo de inatividade incluído no link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["matriz de sistemas suportados"], você pode atualizar o hardware do controlador movendo o armazenamento ou entrando em contato com o suporte técnico. Consulte link:other-references.html["Referências"] para obter o link para _Atualizar movendo volumes ou armazenamento_.




== Processo automatizado de atualização do controlador
...
.O que se segue?
Após tomar conhecimento deste procedimento de atualização do ARL, link:decide-to-use-the-aggregate-relocation-guide.html["decidir se deve usar este procedimento"].
