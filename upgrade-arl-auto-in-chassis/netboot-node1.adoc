...
Não é possível verificar a versão do ONTAP instalada na mídia de inicialização do módulo controlador de substituição, a menos que ele esteja instalado em um chassi e ligado. A versão do ONTAP na mídia de inicialização do sistema de substituição deve ser a mesma que a versão do ONTAP em execução no sistema antigo que você está atualizando e tanto as imagens de inicialização primária quanto de backup na mídia de inicialização devem corresponder. Para verificar a versão mínima do ONTAP compatível com sua atualização, consulte o link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["matriz de sistemas suportados"].
...
* Faça o download dos arquivos de sistema necessários para o seu sistema e da versão correta do ONTAP no _NetApp Support Site_. Consulte link:other-references.html["Referências"] para acessar o _NetApp Support Site_.
...
.Passos
. Consulte link:other-references.html["Referências"] para acessar o _NetApp Support Site_ e baixar os arquivos usados para realizar o netboot do sistema.
...
+
.. Na mensagem seguinte, execute o comando  `wipeconfig`. Em seguida, selecione Enter:
+
[listing]
----
Please choose one of the following:
...
.O que se segue?
Após inicializar o nó1 pela rede, link:cable-node1-for-shared-cluster-HA-storage.html["cabo do nó1 para cluster-HA compartilhado e armazenamento"].
