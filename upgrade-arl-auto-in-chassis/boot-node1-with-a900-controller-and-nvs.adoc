...
[role="lead"]
O Node1 com os módulos de substituição está agora pronto para inicializar. Os módulos de substituição suportados estão listados em link:decide-to-use-the-aggregate-relocation-guide.html#supported-systems-in-chassis["matriz de sistemas suportados"].
...
+
.. Responder `y` para o prompt de confirmação da plataforma mista.
...
+
Insira a senha e as informações de backup que você anotou anteriormente no procedimento. Consulte link:complete-preparation-for-upgrade.html#manage-storage-encryption-using-okm["Gerencie a criptografia de armazenamento usando o Gerenciador de chaves integrado"].
...
. Quando o nó parar no menu de inicialização, reatribua os discos antigos do node1 ao node1 de substituição executando o seguinte comando no node1:
+
...
.O que se segue?
Após inicializar o node1 com os módulos de sistema de substituição, link:verify-node1-installation.html["verifique a instalação do node1"].
