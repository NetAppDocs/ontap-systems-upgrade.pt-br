...
+
Quando o node2 é substituído, o aggr_node_1 entra em funcionamento com o node1 como seu nó principal, em vez do node2.
...
+
Para identificar os agregados que não foram realocados corretamente, consulte a lista de agregados com o proprietário do node1 que você obteve na seção link:prepare-nodes-for-upgrade.html["Prepare os nós para atualização"] e compare-a com a saída do comando acima.

. Compare o resultado da Etapa 1 com o resultado que você capturou para node1 na seção link:prepare-nodes-for-upgrade.html["Prepare os nós para atualização"] e observe quaisquer agregados que não foram realocados corretamente.
