...
== Passo 1: Verifique se o nó1 está em quórum

Verifique se o nó 1 está em quorum. Em seguida, retome a operação de substituição do controlador.
...
. Com a substituição do controlador em estado pausado, prossiga para <<restore_network_configuration_on_node1,Restaure a configuração de rede no node1>>.




== Etapa 2: Restaure a configuração de rede no nó 1
...
.O que se segue?
Após verificar a instalação do node1, link:restore-key-manager-config-upgraded-node1.html["restaurar a configuração do gerenciador de chaves no nó1 atualizado"].
