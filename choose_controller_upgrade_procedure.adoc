...
---
= Escolha um procedimento de atualização para o hardware do controlador AFF, ASA ou FAS
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
Em geral, como você atualiza o hardware do controlador NetApp AFF, ASA ou FAS depende dos modelos de plataforma dos nós originais. Você pode atualizar realocando agregados ou movendo volumes (ambos procedimentos não disruptivos) ou movendo o storage (um procedimento disruptivo). Se você tiver versões diferentes do ONTAP em execução nos nós originais e nos novos, talvez seja necessário realizar uma atualização de software antes de iniciar a atualização de hardware.
...
| Estou executando uma versão de software diferente nos nós originais e nos novos? | Atualize a versão do ONTAP nos nós originais para a versão mais recente suportada. Quando possível, os novos nós devem suportar a versão do ONTAP em execução nos nós originais. Caso isso não seja possível, link:https://docs.netapp.com/us-en/ontap/upgrade/prepare.html["Atualize a versão do ONTAP"^] nos nós originais para a versão mais recente suportada. A diferença de versão entre os nós originais e os novos não pode ser superior a quatro. Por exemplo, ONTAP 9.8 e 9.12.1 são suportados; no entanto, ONTAP 9.8 e 9.13.1 não são suportados. link:https://docs.netapp.com/us-en/ontap/upgrade/concept_mixed_version_requirements.html["Saiba mais sobre clusters ONTAP de versão mista"^]. 


| Quero fazer uma atualização disruptiva? | link:upgrade/upgrade-decide-to-use-this-guide.html["Atualize movendo o armazenamento"]. Isso requer um novo sistema com armazenamento e compartimentos próprios para guardar os dados do sistema original. É possível converter um compartimento de controlador para determinadas atualizações. 
...
* Converta o sistema original em um shelf de discos e mova o armazenamento:
+
** link:upgrade/upgrade_aff_a250_to_aff_a400_ndu_upgrade_workflow.html["Converta AFF A250 em uma shelf de discos e mova o armazenamento para AFF A400"]
** link:upgrade/convert-to-drive-shelf.html["Converta AFF A150, AFF A220 ou FAS2820 em um shelf de unidades e mova o armazenamento para um sistema de substituição compatível"]
...
| Tenho um sistema modular? *Nota*: Em um sistema modular, o controlador não possui discos internos; todo o armazenamento fica em gavetas externas. | link:upgrade-arl/index.html["Atualize usando realocação agregada"]. 
...
| Tenho um sistema integrado e quero atualizá-lo trocando apenas o módulo controlador, mantendo o chassi e os discos existentes? | link:upgrade-arl-auto-in-chassis/index.html["Atualize usando o ARL, mantendo o mesmo chassi, discos e os dados armazenados nos discos"]. 
...
Este exemplo fornece possíveis respostas para uma atualização do controlador FAS9500 para FAS90.
