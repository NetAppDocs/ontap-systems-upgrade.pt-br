...
summary: 'Para concluir o procedimento de atualização por meio da migração do armazenamento, você precisa excluir todas as portas e LIFs não utilizadas dos novos nós, reativar o failover de armazenamento ou a alta disponibilidade, configurar o Service Processor (SP), instalar novas licenças e configurar o AutoSupport. Você também pode precisar configurar a criptografia de armazenamento ou de volume e configurar as portas FC ou CNA.' 
---
