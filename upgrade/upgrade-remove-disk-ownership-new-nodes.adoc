...
. No prompt do menu de inicialização, digite  `9a` e selecione Enter.
+
...
.O que se segue?
Após remover a propriedade dos discos conectados aos novos nós, link:upgrade-reset-default-configuration-node3-and-node4.html["redefina a configuração padrão nos novos nós"].
