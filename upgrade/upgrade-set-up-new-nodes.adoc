...
.Passos
. Ligue a alimentação do node3 e, em seguida, pressione imediatamente Ctrl-C no terminal do console para acessar o prompt do LOADER.
+
...
+
Insira o nome de usuário/senha, se aplicável, ou selecione Enter para continuar.
...
.O que se segue?
Após configurar os novos nós, opcionalmente, link:upgrade-move-internal-drives.html["mover unidades internas do nó original"] ou link:upgrade-convert-node-to-shelf.html["Converta o nó original em uma gaveta de unidades"]. Então, link:upgrade-attach-shelves-reassign-disks.html["Conecte as prateleiras de armazenamento e reatribua a propriedade dos discos"].
