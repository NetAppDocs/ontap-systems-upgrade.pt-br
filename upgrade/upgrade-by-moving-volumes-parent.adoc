...
summary: Se você estiver atualizando por meio da movimentação de volumes, você prepara os nós originais e adiciona os novos nós ao cluster. Você move os volumes para os novos nós, configura as LIFs e remove os nós originais do cluster. A atualização por meio da movimentação de volumes é um procedimento sem interrupções. 
---
= Saiba mais sobre a atualização movendo volumes
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Para atualizar movendo volumes, você prepara os nós originais e adiciona os novos nós ao cluster. Você move os volumes para os novos nós, configura as LIFs e remove os nós originais do cluster.
...
.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-7.png["Sete"]link:upgrade-remove-original-nodes.html["Remova os nós originais do cluster."]
[role="quick-margin-para"]
Após os volumes serem movidos para os novos nós, você remove os nós originais do cluster. Ao remover um nó, a configuração desse nó é apagada e todos os discos são inicializados.
...
.O que se segue?
Após aprender sobre a atualização através da movimentação de volumes, link:upgrade-prepare-when-moving-volumes.html["prepare-se para a atualização"].
